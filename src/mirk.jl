...
function SciMLBase.__init(prob::BVProblem, alg::AbstractSimpleMIRK; dt = 0.0, kwargs...)
    ...
    y = [similar(u0) for i=1:(N+1)]
    y0 = [similar(u0) for i=1:(N+1)]
    residual = [similar(u0) for i=1:(N+1)]
    discrete_stages = [similar(u0) for i=1:stage]
...
function SciMLBase.solve!(cache::SimpleMIRKCache{iip, T}) where {iip, T}
    ...
    sol = [similar(cache.prob.u0) for i=1:cache.N+1]
...
function eval_bc_residual!(cache::SimpleMIRKCache{iip, T}, pt::SciMLBase.StandardBVProblem) where {iip, T}
    cache.prob.f.bc(cache.residual[end], cache.y, cache.p, cache.mesh)
end

function eval_bc_residual!(cache::SimpleMIRKCache{iip, T}, pt::SciMLBase.TwoPointBVProblem) where {iip, T}
    ...
    @views cache.prob.f.bc[2](cache.residual[end][length_a+1:(length_a+length_b)], cache.y[end], cache.p)
end
...
function resid!(cache::SimpleMIRKCache{iip, T}) where {iip, T}
    for i in 1:cache.N
        for r in 1:cache.stage
            ...
            if r > 1
                y_temp += cache.dt*sum(j->cache.x[r, j]*cache.discrete_stages[j], 1:(r-1))
            end
            if iip
        ...
        cache.residual[i] = cache.y[i+1] - cache.y[i] - cache.dt*sum(j->cache.b[j]*cache.discrete_stages[j], 1:cache.stage)
    end
end
